...
Tab switches focus and Esc returns from a conversation to the list.
...
Press `i` to enter the composer. It shows a configurable ghost hint when empty,
...
Select a message and press `R` to reply, or use `R` with no selection for the
latest message. The composer shows the target. Esc cancels the reply first,
...
Use `o` / `g o` to select actionable entries, then Enter to activate. Telegram
...
Mouse support is optional: click a chat or action, right-click a message to
reply, and scroll the pane under the pointer. Settings/account rows are also
clickable. Keyboard equivalents remain available.

## Settings and accounts

`s` opens automatic update checks, release channel, Enter/click download behavior,
