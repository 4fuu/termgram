...
配置了 SOCKS5 代理后，Termgram 会经由代理连接 Telegram。在登录、连接中或
错误屏按 `p` 打开代理编辑器，之后也可以在同一批界面随时修改。编辑器会校验
`socks5://host:port`（凭据可选）并保存到 `settings.conf`。保存后会重启
Telegram 连接；留空表示直连。
...
| `TERMGRAM_PROXY` | SOCKS5 代理地址，如 `socks5://127.0.0.1:9050`；严格模式，见[代理](#代理) |
