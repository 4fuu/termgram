...
// Functions deserialize their bare fields, whereas requests include a constructor.
// Validate and consume it rather than attempting unrelated request layouts.
fn decode_request<R: Deserializable + Identifiable>(body: &[u8]) -> Option<R> {
    let (constructor, fields) = body.split_at_checked(4)?;
    (constructor == R::CONSTRUCTOR_ID.to_le_bytes())
        .then(|| R::from_bytes(fields).ok())
        .flatten()
}
...
fn affected_update(
    ...
    if let Some(request) = request {
        if let Some(request) = decode_request::<tl::functions::channels::DeleteMessages>(request) {
            if let tl::enums::InputChannel::Channel(channel) = request.channel {
                return UpdatesLike::AffectedChannelMessages {
                    affected,
                    channel_id: channel.channel_id,
                    message_ids: request.id,
                };
            }
        } else if let Some(request) = decode_request::<tl::functions::messages::DeleteMessages>(request) {
            return UpdatesLike::Updates(tl::types::UpdateShort {
                ...
                // Same NO_DATE sentinel as the session's AffectedMessages adaptor.
                date: 0,
            }.into());
        }
    }
    ...
    fn process_own_update(
        ...
        match (
            ...
            self.peek_request(msg_id).and_then(|request| {
                decode_request::<tl::functions::messages::SendMessage>(&request.body)
            }),
        ) {
            (
            ...
            (Ok(UpdatesLike::AffectedMessages(affected)), _) => {
                updates.push(affected_update(
                    self.peek_request(msg_id).map(|request| request.body.as_slice()),
                    affected,
                ));
                return;
            }
    ...
    fn affected_updates_keep_payloads_and_pts() {
        for revoke in [false, true] {
            ...
            let UpdatesLike::Updates(tl::enums::Updates::UpdateShort(short)) = affected_update(
                Some(&body),
                tl::types::messages::AffectedMessages { pts: 52, pts_count: 2 },
            ) else { panic!("common deletion must retain its payload") };
            let tl::enums::Update::DeleteMessages(update) = short.update else {
                panic!("expected a deletion update")
            };
            assert_eq!(update.messages, [41, 42]);
            assert_eq!((update.pts, update.pts_count, short.date), (52, 2, 0));
        }
        let body = tl::functions::channels::DeleteMessages {
            channel: tl::types::InputChannel { channel_id: 99, access_hash: 123 }.into(),
            id: vec![42],
        }.to_bytes();
        let UpdatesLike::AffectedChannelMessages { channel_id, message_ids, affected } =
            affected_update(Some(&body), tl::types::messages::AffectedMessages { pts: 77, pts_count: 1 })
        else { panic!("channel deletions must keep their own PTS sequence") };
        assert_eq!(channel_id, 99);
        assert_eq!(message_ids, [42]);
        ...
        assert!(matches!(affected_update(
            Some(&body),
            tl::types::messages::AffectedMessages { pts: 53, pts_count: 1 },
