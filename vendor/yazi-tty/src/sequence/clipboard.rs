...
impl<'a, M> ReadClipboard<'a, M> {
	/// Construct a request without passing through the Lua host.
	pub fn new(mimes: M, pw: &'a str, name: &'a str, primary: bool) -> Self {
		Self { mimes, pw, name, primary, id: "" }
	}

	/// Correlate replies, including failures, across cancellation and timeouts.
...
impl<M: Mimelist> Display for ReadClipboard<'_, M> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		if self.id.len() > 128 || !self.id.bytes().all(|b| b.is_ascii_alphanumeric() || b"-_+.".contains(&b)) {
